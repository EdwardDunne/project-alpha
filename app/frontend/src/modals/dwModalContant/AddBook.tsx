...
const AddBook: React.FC<Props> = ({ setDwModalOpen }) => {
    ...
    const [formData, setFormData] = useState<AddBookFormData>({
        publisher: '', format: '', title: '', author: '', description: '',
        thumbnail_url: '', thumbnail: '', page_count: 0, character: '', team: ''
    });
        ...
        <div className='flex flex-col w-full'>
            ...
            <div className='flex-1 overflow-y-auto max-h-[60vh] px-1 py-3 space-y-3'>
                {[
                    ...
                    <div key={name}>
                        ...
                        <input className={inputClass} type={type} name={name} placeholder={label} onChange={onChange} />
                    </div>
                ))}
                <div>
                    <label className={labelClass} htmlFor='thumbnail'>Thumbnail</label>
                    <input
                        className={inputClass}
                        type='file'
                        name='thumbnail'
                        onChange={e => { if (e.target.files) setFormData({ ...formData, thumbnail: e.target.files[0] }) }}
                    />
                </div>
            ...
            <div className='flex justify-end pt-4 border-t border-gray-100'>
                <button
                    ...
                    form='add-book-form'
                    onClick={e => { e.preventDefault(); addBook(formData, setDwModalOpen) }}
                >
                    Add Book
                </button>
            </div>
