...
const NavBar: React.FC<Props> = ({ isAuthenticated, is_staff, logout }) => {
    ...
    const linkClass =
        "no-underline text-white hover:text-brand transition-colors whitespace-nowrap"
    const mobileLinkClass =
        "no-underline text-gray-700 hover:text-brand transition-colors text-[1.6rem] py-3 px-4 block border-b border-gray-100"
    ...
    return (
        <>
            {/* Nav bar */}
            <nav className="flex justify-between items-center w-full px-4 bg-[#313a46] md:border-b-4 border-solid border-[#1e2130] h-[6rem] shrink-0">
                <div className="flex items-center gap-2">
                    <Link
                        className="text-white no-underline text-[2rem] font-bold whitespace-nowrap"
                        to="/"
                    >
                        Omni Trackers
                    </Link>
                    ...
                    >
                        <button
                            className="w-[2rem] h-[3.6rem] rounded-full bg-[#313a46] hover:text-brand text-white flex items-center justify-center transition-colors"
                            onPointerEnter={handleInfoEnter}
                            onPointerLeave={handleInfoLeave}
                            onClick={() => setInfoOpen((o) => !o)}
                            aria-label="About Omni Trackers"
                        >
                            <InfoOutlinedIcon sx={{ fontSize: "2.3rem" }} />
                        </button>
                        {infoOpen && (
                            <div
                ...
                >
                    <span className="relative w-6 h-6 flex items-center justify-center">
                        {menuOpen ? (
                            <>
                                <span className="absolute w-6 h-[3px] bg-white rounded-full rotate-45" />
                                <span className="absolute w-6 h-[3px] bg-white rounded-full -rotate-45" />
                            </>
                        ) : (
